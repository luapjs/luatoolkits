...
function table.contains(src, element)
    if element and type(src) == "table" then
        for k, v in pairs(src) do
            if v == element then return true, k, v; end
        end
    end
    return false
end
...
function table.contains_func(src, func, obj)
    if func and type(src) == "table" then
        for k, v in pairs(src) do
            if func(v, obj) then return true, k, v; end
        end
    end
    return false
end
...
local function _keys_vals(src, sortFunc, isKey)
    ...
    if type(src) == "table" then
        isKey = isKey == true;
        for k, v in pairs(src) do tb_insert(_ret, isKey and k or v); end
        if sortFunc and #_ret > 1 then tb_sort(_ret, sortFunc); end
    end
    return _ret;
end
...
function table.sub(src, nBegin, nEnd)
    ...
    for i, v in ipairs(src) do
        if i >= nBegin and i <= nEnd then tb_insert(_ret, v); end
    end
    return _ret;
end
...
function table.keyOf(src, value)
    for k, v in pairs(src) do if v == value then return k end end
end

function table.foreach(src, fnvk) for k, v in pairs(src) do fnvk(v, k); end end

function table.foreachArrs(src, fnvk) for k, v in ipairs(src) do fnvk(v, k); end end

function table.foreach_new(src, fnvk)
    local _ret = {}
    for k, v in pairs(src) do _ret[k] = fnvk(v, k); end
    return _ret;
end

function table.filter(src, fnvk)
    local n = {}
    for k, v in pairs(src) do if fnvk(v, k) then n[k] = v end end
    return n
end
...
function clearLT(src, isDeep) return _clear(src, isDeep) end

function table.clear(src, isDeep) return _clear(src, isDeep) end

function table.getVK(src, itKey, itVal)
    if src and itKey and itVal then
        for k, v in pairs(src) do
            if v[itKey] == itVal then return v, k; end
        end
    end
end
    ...
    if src then
        ...
        for _, v1 in pairs(src) do
            for _, v2 in pairs(dest) do
                if v2 == v1 then tb_insert(_ret, v2) end
            end
        end
        return _ret
    end
